...
var policyCmdTestCases = []policyCmdTestCase{
	...
	},
	{
		...
		assert: func(t *testing.T, stdout, stderr string, err error, pathToTmpDir string) {
			warningMsg := "Warning: The effective policy's output may not fully replicate the policy defined in Idira Secrets Manager. If you try to upload the output to Idira Secrets Manager, the upload may fail."
